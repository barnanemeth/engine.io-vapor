...
enum PacketCoder {
    ...
    private enum Constant {
        ...
        static let binaryPackerPrefix: Character = "b"
    }
    ...
    static func decodePacket(from data: String?) throws -> any Packet {
        ...
        if data.first == Constant.binaryPackerPrefix, let data = Data(base32Encoded: String(data.dropFirst())) {
    ...
    static func encodePackets(_ packets: [any Packet]) throws -> String {
        try packets.compactMap { packet -> String? in
            ...
            case let binaryPacket as BinaryPacket:
                return "\(Constant.binaryPackerPrefix)\(binaryPacket.base64String ?? "")"
            default:
